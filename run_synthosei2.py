                ...
                if len(value) == 3:
                    if key in simple_numeric_parameters:
                        ...
                        if key == 'dip_factor_max':
                            # Ensure dip_factor_max stays within reasonable bounds (smooth: 1, faulty: up to 3)
                            resolved_value = min(resolved_value, 3.0)
                        elif key == 'sand_layer_thickness':
                            # Ensure sand_layer_thickness is at least 1 (smooth: 2, faulty: 3)
                        ...
                        if key == 'sand_layer_fraction':
                            ...
                            # With sand_thickness = 2 (smooth): max_fraction = 0.67
                            # With sand_thickness = 3 (faulty): max_fraction = 0.75
                            # Based on smooth config: min=0.05, max=0.25 (safe for sand_thickness >= 2)
                            max_safe_fraction = 0.35  # Updated based on faulty config max
...
def validate_smooth_ranges(config):
    """
    Validate parameter ranges against smooth/example.json values.
    ...
    """
    # Reference ranges from smooth/example.json
    smooth_ranges = {
        'digi': 4,  # Fixed value in smooth
        'initial_layer_stdev': [5.0, 10.0],  # vs [10.0, 30.0] in faulty
        'seabed_min_depth': [20, 50],  # Same in both
        'signal_to_noise_ratio_db': [20.5, 30.5, 35.5],  # vs [5.5, 10.5, 15.5] in faulty
        'bandwidth_low': [10.0, 15.0],  # Same in both
        'bandwidth_high': [60.0, 65.0],  # vs [30.0, 50.0] in faulty
        'bandwidth_ord': 4,  # vs 6 in faulty
        'dip_factor_max': 1,  # vs 3 in faulty
        'min_number_faults': 1,  # Same in both
        'max_number_faults': 6,  # vs 8 in faulty
        'sand_layer_thickness': 2,  # vs 3 in faulty
        'sand_layer_fraction': {'min': 0.05, 'max': 0.25}  # vs {'min': 0.10, 'max': 0.35} in faulty
    }
    
    validated_config = config.copy()
    
    for param, smooth_value in smooth_ranges.items():
        if param in validated_config:
            current_value = validated_config[param]
            
            # For simple numeric parameters, check if they're reasonable for smooth models
            if isinstance(smooth_value, (int, float)) and isinstance(current_value, (int, float)):
                if param == 'dip_factor_max' and current_value > smooth_value:
                    print(f"  Adjusting {param} from {current_value} to {smooth_value} for smooth model")
                    validated_config[param] = smooth_value
                elif param == 'bandwidth_ord' and current_value > smooth_value:
                    print(f"  Adjusting {param} from {current_value} to {smooth_value} for smooth model")
                    validated_config[param] = smooth_value
                elif param == 'sand_layer_thickness' and current_value > smooth_value:
                    print(f"  Adjusting {param} from {current_value} to {smooth_value} for smooth model")
                    validated_config[param] = smooth_value
                elif param == 'max_number_faults' and current_value > smooth_value:
                    print(f"  Adjusting {param} from {current_value} to {smooth_value} for smooth model")
                    validated_config[param] = smooth_value
            
            # For array parameters, check ranges
            elif isinstance(smooth_value, list) and isinstance(current_value, list):
                if param == 'signal_to_noise_ratio_db' and len(current_value) == 3:
                    # Ensure SNR is higher for smooth models
                    if current_value[0] < smooth_value[0]:
                        print(f"  Adjusting {param} SNR range for smooth model")
                        validated_config[param] = smooth_value
                elif param == 'bandwidth_high' and len(current_value) == 2:
                    # Ensure higher frequency content for smooth models
                    if current_value[1] < smooth_value[1]:
                        print(f"  Adjusting {param} for higher frequency content in smooth model")
                        validated_config[param] = smooth_value
                elif param == 'initial_layer_stdev' and len(current_value) == 2:
                    # Ensure lower variability for smooth models
                    if current_value[1] > smooth_value[1]:
                        print(f"  Adjusting {param} for lower variability in smooth model")
                        validated_config[param] = smooth_value
            
            # For dict parameters
            elif isinstance(smooth_value, dict) and isinstance(current_value, dict):
                if param == 'sand_layer_fraction':
                    adjusted = False
                    if current_value.get('max', 0) > smooth_value['max']:
                        validated_config[param]['max'] = smooth_value['max']
                        adjusted = True
                    if current_value.get('min', 0) < smooth_value['min']:
                        validated_config[param]['min'] = smooth_value['min']
                        adjusted = True
                    if adjusted:
                        print(f"  Adjusted {param} range for smooth model: {validated_config[param]}")
    ...
    if ('min_number_faults' in config and 'max_number_faults' in config):
        ...
        if min_faults == 0 and max_faults == 0:
            ...
            if 'dip_factor_max' in config and isinstance(config['dip_factor_max'], (int, float)):
                ...
                elif config['dip_factor_max'] <= 1.0:  # Smooth model range (like example.json with dip_factor_max=1)
                    print(f"  Keeping dip_factor_max at {config['dip_factor_max']} for smooth model")
                else:
                    # Cap at 1.0 for truly smooth models
                    config['dip_factor_max'] = 1.0
                    print(f"  Capped dip_factor_max to 1.0 for smooth model")
...
def load_config(config_path):
    ...
    try:
        ...
        # Check if this is a smooth config and validate ranges
        config_type = determine_config_type(config_path)
        if config_type == 'smooth':
            print("Validating smooth model parameter ranges...")
            config = validate_smooth_ranges(config)
